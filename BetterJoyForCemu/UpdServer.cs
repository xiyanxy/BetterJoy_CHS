...
namespace BetterJoyForCemu {
    class UdpServer {
        ...
        public void Start(IPAddress ip, int port = 26760) {
            if (!Boolean.Parse(ConfigurationManager.AppSettings["启用体感服务"])) {
            ...
            try { udpSock.Bind(new IPEndPoint(ip, port)); } catch (SocketException ex) {
                ...
                form.console.AppendText("无法启动服务. 确保同一时刻仅启动了一个BetterJoy并且没有其他CemuHook应用占用.\r\n");
                return;
            }
            ...
            running = true;
            form.console.AppendText(String.Format("在{0}:{1}启动服务\r\n", ip.ToString(), port));
            StartReceive();
        }
