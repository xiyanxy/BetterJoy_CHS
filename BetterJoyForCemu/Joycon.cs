...
namespace BetterJoyForCemu {
    public class Joycon {
        ...
        int rumblePeriod = Int32.Parse(ConfigurationManager.AppSettings["振动周期"]);
        ...
        public int Attach(byte leds_ = 0x0) {
            ...
            DebugPrint("完成初始化.", DebugType.COMMS);
        ...
        private int ReceiveRaw() {
            ...
            if (ret > 0) {
                ...
                if (ts_en == raw_buf[1] && !isSnes) {
                    form.AppendTextBox("重复的时间戳加入队列.\r\n");
                    DebugPrint(string.Format("重复的时间戳加入队列. TS: {0:X2}", ts_en), DebugType.THREADING);
                }
                ts_en = raw_buf[1];
        ...
        void DetectShake() {
            if (form.shakeInputEnabled) {
                ...
                if (hasShaked && currentShakeTime >= shakedTime + 10) {
                    ...
                    DebugPrint("振动完成", DebugType.SHAKE);
                    hasShaked = false;
                }
        ...
        public void Begin() {
            if (PollThreadObj == null) {
                ...
                form.AppendTextBox("启动PollThread.\r\n");
            } else {
                form.AppendTextBox("PollThread无法启动.\r\n");
            }
        }
